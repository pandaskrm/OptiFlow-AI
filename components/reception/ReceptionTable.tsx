...
export default function ReceptionTable({
  ...
  async function loadReceptions() {
    try {
      ...
      if (!response.ok) {
        throw new Error("Impossible de charger les réceptions.");
      }
  ...
  async function handleNextStatus(item: Reception) {
    ...
    try {
      ...
      const response = await fetch(`/api/receptions/${item.id}`, {
        method: "PATCH",
        headers: {
      ...
      if (!response.ok) {
        throw new Error("Impossible de mettre à jour la réception.");
      }
  ...
  async function deleteReception(id: number) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("Impossible de supprimer la réception.");
      }
          ...
          <tbody>
            {loading ? (
              ...
              receptions.map((item) => {
                ...
                const isCompleted =
                  item.status === RECEPTION_STATUS.COMPLETED;
